---
title: "Toxic Multilabel classification"
author: "Teena"
date: '2018-03-31'
output:
  html_document:
...
#Train and Test split - using XGBoost
```{r}
#partitioning data
...
#Modelling using Naives Bayes

# Using as a baseline for run time - only checking for Toxic labels and compare to XGBoost
...
# part_test_toxic$toxic = as.factor(part_test_toxic$toxic)
# levels(part_test_toxic$toxic) = make.names(unique(part_test_toxic$toxic))
part_test <- dataset[-train_index,]
...
ToxicNB <- naiveBayes(formula, data=part_train)
predNBToxic = predict(ToxicNB,part_test, type = "class")

end_time<-Sys.time()
end_time-start_time

predNBToxic.resp <- ifelse(predNBToxic == "X1", 1, 0)
confusionMatrix(predNBToxic.resp,part_test_toxic, positive = "1")
